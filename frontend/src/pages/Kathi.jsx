...
function Kathi() {
  ...
  // Scroll tracking of kathi screen
  useEffect(() => {
    const handleScroll = () => {
  ...
  const filteredList = useMemo(() => {
    return list.filter((x) =>
      x.food.toLowerCase().includes(filter.toLowerCase())
    );
  }, [list, filter]);
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-gray-800 to-black text-white font-sans relative overflow-x-hidden">
      ...
      {cart.length > 0 && (
        <button 
          ...
          className="fixed top-4 right-4 z-40 bg-yellow-500 hover:bg-yellow-600 text-black px-4 py-2 rounded-full shadow-lg flex items-center gap-2"
        >
      ...
      <div
        ref={headerRef}
        className="relative h-80 bg-[url('https://images.unsplash.com/photo-1565299624946-b28f40a0ae38')] bg-cover bg-center bg-no-repeat mb-12 flex items-center justify-center overflow-hidden"
        style={{ transition: "background-position 0.1s ease-out" }}
      >
        <div className="absolute inset-0 bg-gradient-to-b from-black/80 via-black/60 to-yellow-900/40"></div>
        ...
        <div
          ...
          style={{
            ...
            transition: "transform 0.1s ease-out",
            backgroundImage: `url('https://www.freepnglogos.com/uploads/food-png/food-indian-biryani-transparent-png-images-12.png')`,
            backgroundSize: "contain",
            backgroundRepeat: "no-repeat",
        ...
        >
          <h1 className="text-4xl md:text-6xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-yellow-300 via-yellow-100 to-yellow-400 filter drop-shadow-xl mb-4">
            KATHI MENU
          </h1>
          <p className="text-yellow-100 text-sm md:text-lg max-w-2xl mx-auto mb-6">
            Discover our delicious selection of authentic Kathi rolls and Indian delicacies
          </p>
          ...
          >
            <button type="button" className="text-yellow-600" aria-label="Search menu">
              <svg
                width="20"
                height="20"
                fill="none"
      ...
      <div className="container mx-auto px-4 pb-24 overflow-x-hidden">
        <div className="text-center mb-12">
          <h2 className="text-3xl font-bold mb-2 text-yellow-400">Our Specialties</h2>
          <p className="text-gray-300 max-w-xl mx-auto">
            Handcrafted with love and authentic spices
          </p>
        ...
        {loading ? (
          <div className="flex flex-col items-center justify-center py-20">
            ...
            <p className="text-yellow-400 text-lg">Loading menu items...</p>
          </div>
        ) : (
          <>
            {/* No Items */}
            {filteredList.length === 0 ? (
              <div className="text-center py-16">
                <div
                  ...
                  style={{
                    backgroundImage:
                      "url('https://www.freepnglogos.com/uploads/pizza-png/pizza-png-kaya-massage-therapy-12.png')",
                  }}
                ></div>
                <h3 className="text-2xl font-bold text-yellow-400 mb-2">No items found</h3>
                <p className="text-gray-400">Try adjusting your search or browse our full menu</p>
                <button
                  onClick={() => setFilter("")}
                  className="mt-4 px-6 py-2 bg-yellow-600 hover:bg-yellow-700 rounded-full text-white font-medium transition-colors"
                >
                  View All Items
                </button>
              </div>
              ...
              <div className="relative">
                ...
                <div
                  ...
                  style={{
                    backgroundImage:
                      "url('https://www.freepnglogos.com/uploads/food-png/food-indian-transparent-png-pictures-icons-and-png-7.png')",
                    backgroundSize: "contain",
                    backgroundRepeat: "no-repeat",
                    transform: `translateY(${scrollY * 0.15}px) rotate(${scrollY * -0.03}deg)`,
                  }}
      ...
      {scrollY > 200 && (
        <button
          ...
          className="fixed bottom-6 right-6 z-50 p-3 bg-yellow-500 hover:bg-yellow-600 text-black rounded-full shadow-lg transition"
      ...
      {/* Cart Modal */}
      {isCartOpen && <CartModal cart={cart} setCart={setCart} outlet="kathi" onClose={() => setIsCartOpen(false)} />}
    </div>
  );
}
